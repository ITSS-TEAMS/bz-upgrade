...
$PACKAGE EB.BZLocalRoutinesEnq1
        SUBROUTINE BZ.DC.CONTROL.COMPTE.2
$USING EB.API
...
* $INSERT I_F.BZ.COFFRE.LOYER - Not Used anymore;$NAU"
    F.DC.NAU = ""
...
*-------------------------------------------------------------
MAIN:
      IDEB.SystemTables.getVFunction()EW
      COMPTE = R.NEW(DC.DC.ACCOUNT.NUFT.AdhocChargeRequests.AcChargeRequest.ChgRecordStatusSE = R.NEW(DC.DC.CURRENCY)
    *RECHERCHE DESFT.AdhocChargeRequests.AcChargeRequest.ChgRecordStatusAPTURE APPARTENANT AU MEME BATCH  (ET AU MEME DEPARTEMENT CODE)
    ...
    *TRAIEB.DataAccess.OpfENT POUR DATA.CAPTURE
    SEL.LIST = ""
    SEL.NBR =""
    SEL.ERR =""
    SEL.CMD = "SELECT ":FN.DC :" CURRENCY ACCOUNT.NUMBER WITH @ID LIKE ":DC.SIMILAIRE:"... TO 1"
    * pour genererEB.SystemTables.getRNew()cFT.AdhocChargeRequests.AcChargeRequest.ChgRelatedRef      *TEXT = "SELECT COMMANDE " : SEL.CMD
        *EB.DataAccess.FReadEM
    AUTHORIZED = "TRUE"
   * EXECUTE SEL.CMD ;* Executer le SELECT
   SEL.ERR = '' ; SEL.CNT = '' ; SEL.LIST = ''
	CALL EB.READLIST(SELEB.BZLocalTable1.BzCoffreLoyer.BzCoffreLoyerRefLoyerEL.CNT,SEL.ERR)  ;*EXECUTE/READLIST converted to EB.READLIST.
	
*    READLISEB.SystemTables.setE("FALSE")
    IF AUTHORIZED NE "FALSE" THEN
    SEL.NBR = DCOUNTEB.BZLocalTable1.BzCoffreLoyer.BzCoffreLoyerRefLoyer  ;*
    * pour generer des commentaires
        *TEXT = "EB.DataAccess.FWrited'entrees   " : SEL.NBR / 2
        *CALL REM
...
MAIN:
   ...
   * EXECUTE SEL.CMD ;* Executer le SELECT
   SEL.ERR = '' ; SEL.CNT = '' ; SEL.LIST = ''
	CALL EB.READLIST(SEL.CMD,SEL.LIST,'',SEL.CNT,SEL.ERR)  ;*EXECUTE/READLIST converted to EB.READLIST.
...
*    READLIST SEL.LIST FROM 1 ELSE AUTHORIZED = "FALSE"
    ...
    *CONTROL SUR LES DEVISES. PAS DE DEVISE DIFFERENTE DES AUTRES
    *CONTROL SUR LES CATEGORIES DE COMPTE:
    *   BloFT.AdhocChargeRequests.AcChargeRequest.ChgRelatedRefres comptables DC faisant intervenir un compte hors bilan contre un compte bilan
    *LesEB.SystemTables.setE()*-----------------------------------------------------------------------------
*-----------------------------------------------------------------------------
* <Rating>514</Rating>
*-----------------------------------------------------------------------------
$PACKAGE EB.BZLocalRoutinesEnq1
