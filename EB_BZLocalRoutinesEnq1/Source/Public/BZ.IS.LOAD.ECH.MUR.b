...
********EB.SystemTables.getVFunction()        FN.LD.LOANS.AND.DEPOSITS = 'F.LD.FT.AdhocChargeRequests.AcChargeRequest.ChgRecordStatusS'
        F.LD.LOANS.AND.DEPOSITS = ''
    FT.AdhocChargeRequests.AcChargeRequest.ChgRecordStatusD.DEPOSITS = ID.NEW
        ...
        CEB.DataAccess.Opf.ETUDE = R.LD.LOANS.AND.DEPOSITS<LD.CHRG.AMOUNT,1>
        CEB.DataAccess.OpfNT = R.LD.LOANS.AND.DEPOSITS<LD.CUSTOMER.ID>
        CALL F.READ(FN.CUSTOMER,CLIENT,R.CUSTOMER,F.CUSTOMER,E.CUSTOMER)

            NOM.CLIENT = R.CUSTOMER<EB.CUS.SHORT.NAME>

*    CALL GET.LOC.EB.SystemTables.getRNew()LFT.AdhocChargeRequests.AcChargeRequest.ChgRelatedRefOSITS","SEC.DEP.AMT",N.LD.ACOMPTE)   //CHANGED GET.LOC.REF TO MULTI.GET.LOC.REF
*    CALL GET.LOC.REF("LD.LOANS.AND.DEPOSITS","TIEG",N.TIEG)

       EB.DataAccess.FReadICATION.NAME = "LD.LOANS.AND.DEPOSITS"
        YFIELD.NAME ="SEC.DEP.AMT":@VM:"TIEG"
        LREF.POS = ''
        CALL MULTI.GET.LOC.REB.BZLocalTable1.BzCoffreLoyer.BzCoffreLoyerRefLoyerME, YFIELD.NAME, LREF.POS)
        N.LD.ACOMPTE = LREF.POS<1, 1>
        N.TIEB.SystemTables.setE(LREF.POS<1, 2>)

        CATEG = R.LDEB.BZLocalTable1.BzCoffreLoyer.BzCoffreLoyerRefLoyerS<LD.EB.SystemTables.getIdNew()RY>
        ACOMPTE = 0
        IF CATEG EB.DataAccess.FWrite55' THEN
...
*    CALL GET.LOC.REF("LD.LOANS.AND.DEPOSITS","SEC.DEP.AMT",N.LD.ACOMPTE)   //CHANGED GET.LOC.REF TO MULTI.GET.LOC.REF
*    CALL GET.LOC.REF("LD.LOANS.AND.DEPOSITS","TIEG",N.TIEG)
        ...
        END
        MONATNT.FIN = R.LD.LOANS.AND.DEPOSITS<LD.AMOUNT> + ACOMPTE
        TIEFT.AdhocChargeRequests.AcChargeRequest.ChgRelatedRefND.DEPOSITS<LD.LOCAL.REF,N.TIEG>
        EB.SystemTables.setE(R.LD.LOANS.AND.DEPOSITS<LD.RESIDUAL.VALUE>)
        CATEG = R.LD.LOANS.AND.DEPOSITS<LD.CATEGORY>
    ...
    R.NEW(BZ.ECH.DETAIL.TOTAL.COUT) = SUM( R.RECORD<18>) + ABS(R.RECORD<17,1>)
