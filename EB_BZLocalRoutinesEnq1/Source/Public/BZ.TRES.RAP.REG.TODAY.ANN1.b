...
$PACKAGE EB.BZLocalRoutinesEnq1
    SUBROUTINE BZ.TRES.RAP.REG.TODAY.ANN1
$USING EB.API
...
*ZIT-UPG-R13-R19-CONVERT TO CHANGE;SM,FM,VM TO @SM,@FM,@VM;INITIALISED VARIABLES;GET.LOC.REF TO MULTI.GET.LOC.REF
*				-F.READ TO CACHE.READ;OPERATIONS TO OPERANDS
*-------------------------------------------------------
...
INIT:
    ...
    FN.FX.NAU = "F.FOREX$NAU"
    F.FX.NAU=""

    FN.FT.AdhocChargeRequests.AcChargeRequest.ChgRecordStatus
    F.CURR=""
	CURR.REC=""
	CURR.ERR=""
	FT.AdhocChargeRequests.AcChargeRequest.ChgRecordStatus="F.CURRENCY$HIS"
    F.CURR.HIS=""
	CURR.HIS.REC=""
	CURR.HIS.ERR=""
...
*        CONVERT SM TO " " IN Y.CURRENCY
    ...
    END
EB.DataAccess.Opf   FN.EB.DataAccess.Opf.MVMT = "F.POS.MVMT.TODAY"
    F.POS.MVMT = ""
	R.POS.RECORD.MVMT=""
	POS.MVMT.ERR=""
	R.POS.RECOEB.SystemTables.getRNew()FT.AdhocChargeRequests.AcChargeRequest.ChgRelatedRef.DataAccess.FRead.HIST = "F.POS.MVMT.HIST"
    F.POS.MVMT.HIST = ""
	R.POS.RECORD.MVMT.HIST=""
	POS.MVMT.ERR=""
	
    NBRE.TRAIT.DETEB.BZLocalTable1.BzCoffreLoyer.BzCoffreLoyerRefLoyerRECEB.SystemTables.setE(0)
    NBRE.DEVISE = 0

    Y.YESTERDAY = EB.BZLocalTable1.BzCoffreLoyer.BzCoffreLoyerRefLoyerAccess.FWriteDAY,'-1W')

    Y.YESTERDAY.YEARS = Y.YESTERDAY[1,4]

    Y.YEEB.SystemTables.setAf()*
$PACKAGE EB.BZLocalRoutinesEnq1
    SUBROUTINE BZ.TRES.RAP.REG.TODAY.ANN1
$USING EB.API
$USING EB.SystemTables
...
*        CONVERT SM TO " " IN Y.CURRENCY
    ...
    Y.YESTEFT.AdhocChargeRequests.AcChargeRequest.ChgRelatedRefSTERDAY[7,2]:Y.YESTERDAY[5,2]:Y.YESTERDAY[3,2]
...
*        CONVERT SM TO " " IN Y.CURRENCY
    ...
    Y.YESTERDAY.AFF = Y.YESTERDAY[7,2]:Y.YESTERDAY[5,2]:Y.YESTERDAY[3,2]
...
******************
    ...
    Y.FILE.NAME = "ANNEXE1-POSITION-CHANGE-DU-":TODAY:".txt"

* EXECUTE 'SH -c "rm ./' : Y.REP.IN :'/' : Y.FILE.NAME:'"' CAPTURING DELETE.RESULT
...
*****************
    ...
    Y.LIGNE.HDR.1 = "JOURNEE DU : ":Y.YESTERDAY.AFF
    Y.LIGNE.HDR = "":Y.SPACE
    Y.LIGNE.HDR :="Devise":Y.SPACE
    Y.LIGNE.HDR :="Type operation":Y.SPACE
    Y.LIGNE.HDR :="Date saisie":Y.SPACE
    Y.LIGNE.HDR :="Heure saisie":Y.SPACE
    Y.LIGNE.HDR :="Achat":Y.SPACE
    Y.LIGNE.HDR :="Vente":Y.SPACE
    Y.LIGNE.HDR :="Position":Y.SPACE

    Y.LIGNE.HDR = Y.LIGNE.HDR.1 : @FM : Y.LIGNE.HDR
...
OPENFILES:
    ...
    CALL OPF(FN.FX,F.FX)
    CALL OPF(FN.FX.NAU,F.FX.NAU)

    RETURN

GET.DATE.TIME:

    Y.TXN.REFERENCE = Y.TXN.REFERENCE[1,12]
    CALL F.READ(FN.FX,Y.TXN.REFERENCE,R.FX,F.FX,FX.ERR)

    IF FX.ERR THEN
        CALL F.READ(FN.FX.NAU,Y.TXN.REFERENCE,R.FX,F.FX.NAU,FX.ERR)

        IF R.FX THEN
            Y.DATE.TIME = R.FX<FX.DATE.TIME>[7,10]
        END
...
************
        ...
        Y.POSITION.INIT= FMT(Y.OPENNING.BALANCE,"R#20")
        Y.SOMME.ACHAT = FMT(Y.SOMME.MVMT.ACHAT.RST,"R#20")
        Y.SOMME.VENTE = ABS(Y.SOMME.MVMT.VENTE.RST)
        Y.SOMME.VENTE = FMT(Y.SOMME.VENTE,"R#20")
        Y.POSITION.FINAL = FMT(Y.ONLINE.BALANCE.SUM,"R#20")
        Y.TXN.DEVISE = TAB.AMOUNT<COMPTEUR.DEV,3>
        Y.SENS.OPERATION = TAB.AMOUNT<COMPTEUR.DEV,2>

        Y.TXN.REFERENCES = TAB.AMOUNT<COMPTEUR.DEV,4>

        Y.LIGNE = Y.DEVISE:Y.SPACE
        Y.LIGNE :=Y.SPACE:Y.SPACE:Y.SPACE:Y.SPACE
        Y.LIGNE :=Y.SOMME.ACHAT:Y.SPACE
        Y.LIGNE :=Y.SOMME.VENTE:Y.SPACE
        Y.LIGNE :=Y.POSITION.FINAL

        Y.LIGNE :=Y.RET.LIGNE

        Y.LIGNE :=Y.SPACE:Y.SPACE:"Position veille":Y.SPACE:Y.SPACE:Y.SPACE:Y.SPACE:Y.SPACE:Y.POSITION.INIT

        Y.LIGNE :=Y.RET.LIGNE

*        CONVERT SM TO FM IN Y.TXN.DEVISE
*        CONVERT SM TO FM IN Y.SENS.OPERATION
*        CONVERT SM TO FM IN Y.TXN.REFERENCES
		CHANGE @SM TO @FM IN Y.TXN.DEVISE
        CHANGE @SM TO @FM IN Y.SENS.OPERATION
        CHANGE @SM TO @FM IN Y.TXN.REFERENCES
        NBRE.AMT = DCOUNT(Y.TXN.DEVISE,@FM)

        IF NBRE.AMT GE 1 THEN

            FOR COMPTEUR.AMT = 1 TO NBRE.AMT

                Y.OPERATION.TMP =  Y.TXN.DEVISE<COMPTEUR.AMT>
                Y.SENS.TMP = Y.SENS.OPERATION<COMPTEUR.AMT>
                Y.TXN.REFERENCE = Y.TXN.REFERENCES<COMPTEUR.AMT>
                GOSUB GET.DATE.TIME

                IF Y.SENS.TMP MATCHES "P":@VM:"S*" THEN

                    Y.LIGNE :=Y.SPACE:Y.SPACE:"Operation Jour":Y.SPACE:Y.YESTERDAY.AFF:Y.SPACE:Y.DATE.TIME:Y.SPACE:ABS(Y.OPERATION.TMP):Y.SPACE::Y.SPACE:ABS(Y.OPERATION.TMP):Y.SPACE

                END ELSE

                    Y.LIGNE :=Y.SPACE:Y.SPACE:"Operation Jour":Y.SPACE:Y.YESTERDAY.AFF:Y.SPACE:Y.DATE.TIME:Y.SPACE:Y.SPACE:(Y.OPERATION.TMP):Y.SPACE:(Y.OPERATION.TMP*-1):Y.SPACE
        ...
        END ELSE
            ...
            Y.LIGNE :=Y.SPACE:Y.SPACE:"Operation Jour":Y.SPACE:Y.SPACE:Y.SPACE:Y.SPACE:Y.SPACE:0
...
****************
    ...
    Y.MONTANT.DET = R.POS.RECORD.MVMT<PSE.AMOUNT.FCY>

    Y.REFERENCE = R.POS.RECORD.MVMT<PSE.OUR.REFERENCE>
    ...
    IF NOT(R.POS.RECORD.MVMT<PSE.CURRENCY> MATCHES "":@VM:"TND":@VM:"TNC") THEN

        TAB.AMOUNT<NBRE.DEVISE,1> = R.POS.RECORD.MVMT<PSE.CURRENCY>

        TAB.AMOUNT<NBRE.DEVISE,2,-1> = Y.SENS.OP

        TAB.AMOUNT<NBRE.DEVISE,3,-1> = Y.MONTANT.DET

        TAB.AMOUNT<NBRE.DEVISE,4,-1> = Y.REFERENCE
...
*    CONVERT FM TO CHARX(10) IN TABLEAU.FINAL
	CHANGE @FM TO CHARX(10) IN TABLEAU.FINAL
    WRITESEQ TABLEAU.FINAL TO FP.FILE.NAME ELSE
