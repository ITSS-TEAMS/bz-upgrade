* @ValidationCode : MjotMTIyODc4OTU3OTpDcDEyNTI6MTYwMDU2MDcxMDc1MDpBZG1pbmlzdHJhdGV1cjotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMF9TUDQuMDotMTotMQ==
...
* Main Program Loop
    ...
    UNTIL (EB.SystemTables.getMessage() EQ 'RET')
        ...
        END ELSE
            ...
            IF V$ERROR THEN GOSUB MAIN.REPEAT    ;*GOTO MAIN.REPEAT

            EB.TransactionControl.RecordRead()

            IF EB.SystemTables.getMessage() EQ 'REPEAT' THEN
                GOSUB MAIN.REPEAT    ;*GOTO MAIN.REPEAT
            END
...
REM > CALL XX.CHECK.FIELDS
    ...
    BEGIN CASE
        ...
        CASE EB.SystemTables.getAf() EQ 9
            ...
            IF EB.SystemTables.getComi() EQ 1 THEN EB.SystemTables.setE("ERROR TEST1")



        CASE EB.SystemTables.getAf() EQ 10
            IF EB.SystemTables.getComi() EQ 2 THEN EB.SystemTables.setE("ERROR TEST2")
...
*
    IF EB.SystemTables.getEndError() THEN
        T.ETEXT = ''
        A = 1
        LOOP UNTIL T.ETEXT<A> NE "" DO A = A+1 ; REPEAT
        T.SEQU = "D"
        T.SEQU<-1> = A
        V$ERROR = 1
...
* Validation of function entered.  Sets V$FUNCTION to null if in error.
    ...
    IF INDEX('V',tmp.V$FUNCTION,1) THEN
        EB.SystemTables.setVFunction(tmp.V$FUNCTION)
        EB.SystemTables.setE('EB.RTN.FUNT.NOT.ALLOWED.APP')    ;*   old EB.ERROR ID must be changed to new ID
...
INITIALISE:
    ...
    BROWSER.PREVIEW.ON = (EB.Interface.setOfsMessage('PREVIEW'))  ;*EN_10002679 - S/E
